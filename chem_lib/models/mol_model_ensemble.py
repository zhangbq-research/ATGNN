...
from .encoder_ensemble import GNN_Encoder
...
class EnsembleContextAwareRelationNet(nn.Module):
    def __init__(self, args):
        super(EnsembleContextAwareRelationNet, self).__init__()
        ...
        if args.pretrained:
            ...
            model_file_pt = model_file.format('supervised_contextpred')
            print('load finetuning model from', model_file)
            model_file_ft = model_file.format('best_gin_{}_{}'.format(args.dataset, args.n_shot_train))
            # model_file_ft = model_file.format('supervised_contextpred')
            self.mol_encoder.from_pretrained(model_file_pt, model_file_ft, self.gpu_id)
    ...
    def forward(self, s_data, q_data, s_label=None, q_pred_adj=False):
        ...
        # q_logits_pn = torch.exp(self.scale)*F.cosine_similarity(q_emb.unsqueeze(1).expand(-1, prototypes.shape[0], -1), 
        #                                             prototypes.unsqueeze(0).expand(q_emb.shape[0], -1, -1), dim=-1)
        # s_logits_pn = torch.exp(self.scale)*F.cosine_similarity(s_emb.unsqueeze(1).expand(-1, prototypes.shape[0], -1), 
        #                                             prototypes.unsqueeze(0).expand(s_emb.shape[0], -1, -1), dim=-1)

        q_logits_pn = torch.sum(q_emb.unsqueeze(1).expand(-1, prototypes.shape[0], -1)* 
                                                    prototypes.unsqueeze(0).expand(q_emb.shape[0], -1, -1), dim=-1)
        s_logits_pn = torch.sum(s_emb.unsqueeze(1).expand(-1, prototypes.shape[0], -1)* 
                                                    prototypes.unsqueeze(0).expand(s_emb.shape[0], -1, -1), dim=-1)


        return s_logits, q_logits, adj, s_node_emb, s_logits_pn, q_logits_pn
    ...
    def forward_query_list(self, s_data, q_data_list, s_label=None, q_pred_adj=False):
        ...
        q_emb_list = [self.mol_encoder(q_data.x, q_data.edge_index, q_data.edge_attr, (q_data.batch, q_data.y, False))[0] for q_data in
                      q_data_list]
